...
function [resultsMass,resultsNum] = main(MP)
    ...
    if CAL_COEFFS 
        ...
        filename1 = [currentDir,'\','coefficients\',filePrefix,'coag_',KERNEL,'_',num2str(ND),'_',num2str(NT),'_',num2str(binsPer2), '.mat'];
        if isfile(filename1)
           load(filename1);
    ...
    if CAL_SINK_PARAMS
        ...
        filename2 = [currentDir,'\','coefficients\',filePrefix,'sinkParams_',KERNEL,'_',num2str(ND),'_',num2str(NT),'_',num2str(binsPer2),'.mat'];
        if isfile(filename2)
           load(filename2);
        else
    ...
    if exist([currentDir,filesep,'results']) 
